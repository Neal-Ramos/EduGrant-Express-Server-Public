...
export const uploadWasabi = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const file: Express.Multer.File | undefined = (req.files as Express.Multer.File[]).find(
      (f) => f.fieldname === 'IMG',
    );
    if (!file) {
      res.status(404).json();
...
export const getAnnouncementsPublic = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const { page, dataPerPage, sortBy, order, status } = (
