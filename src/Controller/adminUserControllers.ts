...
import {
  ...
  prismaUpdateHeadAccount,
  prismaUpdateStaffAccount,
  prismaUpdateWebTour,
...
export const editHead = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { address, fName, lName, mName, gender } = (
      req as Request & { validated: editHeadZodType }
    ).validated.body;
    ...
    const update = await prismaUpdateHeadAccount(
      HeadID,
      address,
      fName,
      lName,
...
export const editStaff = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { ownerId, fName, lName, mName, email, password, validate } = (
      req as Request & { validated: editStaffZodType }
    ).validated.body;
    const profileImg: Express.Multer.File | undefined = (req.files as Express.Multer.File[]).find(
      (f) => f.fieldname === 'profileImg',
    );
    const HeadId = Number(req.tokenPayload.accountId);
    ...
    const updateStaff = await prismaUpdateStaffAccount(
      ownerId,
      fName,
      lName,
      mName,
    ...
    res
      .status(200)
      .json({ success: true, message: 'Staff Account Updated!', updatedStaff: safeData });
  } catch (error) {
    next(error);
  }
};
export const editStaffInfoAccount = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const { fName, mName, lName } = (req as Request & { validated: editStaffInfoZodType }).validated
      .body;
    const file = (req.files as Express.Multer.File[]).find((f) => f.fieldname === 'profileImg');
    ...
    const updatedStaffInfo = await prismaUpdateStaffInfo(
      accountId,
      fName,
      mName,
      lName,
    ...
    res
      .status(200)
      .json({ success: true, message: 'Staff Info Updated!', updatedStaffInfo: safeDate });
  } catch (error) {}
};
export const editStaffCredSendAuthCode = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const { oldPass, newPass } = (req as Request & { validated: editStaffCredSendAuthCodeZodType })
      .validated.body;
...
export const changeStaffCred = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const accountId = Number(req.tokenPayload.accountId);
    const { code, newEmail, oldPass, newPass } = (
      req as Request & { validated: changeStaffCredZodType }
    ).validated.body;
    ...
    const updateStaffLoginCred = await prismaUpdateAccountLoginCredentials(
      accountId,
      undefined,
      newEmail,
      newPass,
