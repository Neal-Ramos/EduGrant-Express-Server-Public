...
export const prismaCreateStudentAccount = async (
  fourPs: boolean,
  dswd: boolean,
  civilStatus: string,
  studentType: string,
...
export const prismaUpdateStudentAccount = async (
  ...
  pwd?: string,
  indigenous?: string,
  profileImg?: ResponseUploadSupabase,
...
export const prismaUpdateHeadAccount = async (
  accountId: number,
  address?: string,
  firstName?: string,
  lastName?: string,
...
export const prismaUpdateStaffAccount = async (
  accountId: number,
  fName?: string,
  lName?: string,
  mName?: string,
  ...
  hashedPassword?: string,
  validate?: string,
  profileURL?: ResponseUploadSupabase,
): Promise<prismaUpdateStaffAccountType | null> => {
  const newValidate = validate === 'true' ? true : validate === 'false' ? false : undefined;
  const update = await prisma.account.update({
    where: {
      accountId: accountId,
    },
...
export const prismaGetHeadDashboard = async (): Promise<{}> => {
  const [
    ...
    scholarship,
    applcationCount,
    approvedApplcationCount,
    pendingApplcationCount,
    activeScholarshipCount,
    PWDApplicationCount,
    indiginousApplicationCount,
    GeneralCount,
    applicationCountPerInsti,
    annoucement,
    applicationCountToday,
    applicationPendingToday,
    applicationApprovedToday,
    scholarshipCountToday,
  ] = await Promise.all([
    ...
    prisma.$queryRaw<{ institute: string; applicationCount: number }[]>`
...
export const prismaHEADUpdateStudentAccount = async (
  ownerId: number,
  email?: string,
