...
export const prismaCreateScholarship = async (
  ...
  scholarshipDocuments: DocumentEntry[],
  scholarshipAmount: string | undefined,
  scholarshipLimit: number | undefined,
  gwa: number | undefined,
  accountId: number,
  sponsorResult: ResponseUploadSupabase,
  coverResult: ResponseUploadSupabase,
  formResult: ResponseUploadSupabase | undefined,
  isForInterview: string,
): Promise<Scholarship> => {
...
export const prismaGetScholarship = async (
  page?: number,
  dataPerPage?: number,
  sortBy?: string,
  order?: string,
...
export const prismaGetRenewScholarship = async (
  ...
  filters: { id: string; value: string }[] | undefined,
  accountId: number,
): Promise<{
  scholarship: Scholarship[];
  totalCount: number;
...
export const prismaSearchScholarshipTitle = async (
  page?: number,
  dataPerPage?: number,
  search?: string,
  sortBy?: string,
...
export const prismaRenewScholarship = async (
  accountId: number,
  scholarshipId: number,
  updatedRequirements: object,
  renewDeadline: Date,
  isForInterview: 'false' | 'true',
  ...
  while (true) {
    // Create Bulk Notifications for Renew Applications
...
export const prismaFiltersScholarship = async (status: string | undefined): Promise<{}> => {
  ...
  return {
    scholarship: [...new Set(filterScholarship.map((e) => e.title))],
    phase: [...new Set(filterScholarship.map((e) => e.phase))],
    provider: provider.map((e) => e.name),
  };
};
...
export const prismaStudentCountsInToken = async (
  accountId?: number,
): Promise<{
  ...
  applicationCountPerStatus: Record<string, number>;
