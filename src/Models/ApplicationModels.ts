...
import {
  ApplicationWithScholarshipType,
  familyBackgroundType,
  prismaAcceptForInterviewType,
  prismaApproveApplicationType,
  prismaCreateApplicationType,
  prismaDeclineApplicationType,
  prismaGetApplicationType,
  prismaRenewApplicationType,
...
export const prismaGetAllApplication = async (
  status?: string,
  page?: number,
  dataPerPage?: number,
  sortBy?: string,
  ...
  const [applications, applicationsCount, rawStatusCounts] = await Promise.all([
    prisma.application.findMany({
      ...
      include: {
        Student: {
          include: {
            Account: { select: { accountId: true, schoolId: true, email: true, role: true } },
            Application: { select: { applicationId: true } },
          },
        },
        Scholarship: { include: { Scholarship_Provider: true, ISPSU_Head: true } },
...
export const prismaSearchApplication = async (
  search?: string,
  status?: string,
  sortBy?: string,
  order?: string,
...
export const prismaApproveApplication = async (
  applicationId: number,
  accountId: number,
  rejectMessage: {},
  scholarshipPhase: number,
): Promise<{
  Application: prismaApproveApplicationType;
  ...
  const transac = await prisma.$transaction(async (tx) => {
    ...
    await Promise.all(
      groupGovApplications.map(async (e) => {
  ...
  return {
    Application: transac.approveApplication,
    BlockedApplications,
    notification: transac.notification,
  };
};
export const prismaAcceptForInterview = async (
  applicationId: number,
  accountId: number,
  rejectMessage: {},
  scholarshipPhase: number,
): Promise<{
  interviewApplication: prismaAcceptForInterviewType;
  notification: Student_Notification;
}> => {
  const transac = await prisma.$transaction(async (tx) => {
...
export const prismaDeclineApplication = async (
  applicationId: number,
  accountId: number,
  rejectMessage: {},
  scholarshipPhase: number,
): Promise<{
  declineApplication: prismaDeclineApplicationType;
  notification: Student_Notification;
}> => {
  const transac = await prisma.$transaction(async (tx) => {
...
export const prismaCreateApplication = async (
  fileRequirements: RecordApplicationFilesTypes,
  supabasePath: string[],
  accountId: number,
  scholarshipId: number,
...
export const prismaUpdateApplicationSubmittedFiles = async (
  applicationId: number,
  accountId: number,
  scholarshipId: number,
  fileRequirements: RecordApplicationFilesTypes,
  allApplicationPaths: string[],
): Promise<Application | null> => {
...
export const prismaBlockApplicationByOwnerId = async (ownerId: number): Promise<number> => {
  ...
  await Promise.all(
    Object.values(records).map(async (e) => {
